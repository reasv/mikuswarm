 ...
 * CONCURRENCY-AND-RATE-LIMITING §6.2 / Decision D). On a 200 the resume ran to
 * completion (`{ sessionId, status: "completed" }`); a 409 (not resumable /
 * resume failed again, with the resulting `sessionStatus`) is mapped to a
 * thrown `HttpError` by the API client before this schema is reached.
 */
