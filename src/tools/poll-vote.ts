...
export function createPollVoteTool(context: PollVoteToolContext): AgentTool {
  ...
  return {
    ...
    parameters: Type.Object({
      poll_event_id: Type.String({ description: `${t.messageIdFmt} of the poll to vote in.` }),
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (!context.channelClient.votePoll) {
        return {
          content: [{ type: "text", text: "error: poll voting is not supported on this channel." }],
          details: null,
        };
      }

      try {
        const result = await context.channelClient.votePoll!({
          pollExternalId: args.poll_event_id.trim(),
          answerIds: args.answers,
        });

        return {
          content: [{ type: "text", text: `vote submitted: ${result.externalId}` }],
          details: result,
        };
      } catch (err) {
