...
export interface InitialBackfillOptions {
  ...
  maxMessages: number;
  /** Stop once a fetched page reaches this far before the oldest known event (ms). */
  windowMs: number;
 ...
 * floor (`oldestStored - windowMs`), history exhausted, or the timeout — the
 * trigger is held until then.
 *
...
export async function performInitialBackfill(
  ...
  const oldestStored = storage.getOldestEventTimestamp(timelineKey) ?? Date.now();
  const windowFloor = oldestStored - windowMs;
  ...
  while (true) {
    ...
    before = page.nextBatch ?? undefined;
    if (!before) {
      result.exhausted = true;
      break;
    }
