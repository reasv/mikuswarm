...
First, they are single-owner by design. The security model pairs the agent to its owner's accounts and confines it to DMs by default — reasonable, since it is operating the owner's computer — so running one in a room full of other people means switching every one of those protections off and hoping the sandbox holds.
...
A group chat needs the opposite premise: **the conversation is not the session.** A room moves on its own, most of what happens has nothing to do with the agent, and the agent is a participant, not the protagonist.

So MikuSwarm is chat-first. Messages are ingested verbatim — the bot's own included — into a database built for chat, and summarization runs continuously over the chatlog itself, whether or not the agent is doing anything, so compaction is never something a reply has to wait for. When something triggers the agent, that trigger spawns its own session: context is assembled fresh from the shared timeline, the session does its work, and it speaks only through an explicit `send_message` tool — raw model output, thinking included, never reaches the room. That is the swarm in the name: every trigger is an independent, parallel session over one shared history. When a session ends, its thinking, tool calls, and dead ends evaporate; the messages it chose to send are the only artifact. However much work an answer took, the room's history stays clean, and a slow research dig for one person never blocks a quick answer for another.

The honest trade-off: sessions are normally one rollout. Replying to a bot message resumes the session that produced it when there is real state to carry forward, but long, iterative, session-continuous work — pair-programming with the room bot — is deliberately out of scope. In exchange, the usage pattern a public chat agent actually sees, where the answer is the product, maps perfectly onto one session per question.
