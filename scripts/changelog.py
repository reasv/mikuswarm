#!/usr/bin/env python3
"""Validate MikuSwarm's structured CHANGELOG.md and extract a release's notes.

The release workflow runs `validate` to fail the build on a malformed changelog,
then `extract --tag vX.Y.Z` to pull that version's section verbatim and use it as
the GitHub release body. Requires only the standard library, so CI runs it with a
plain `python3` and no dependency install.
"""
...
def release_for_tag(releases: list[Release], tag: str) -> Release:
    ...
    if len(matches) != 1:
        raise ChangelogError(
            f"expected exactly one non-empty changelog section for {tag}"
        )
    return matches[0]
...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("validate", help="fail if CHANGELOG.md is malformed")
    extract = sub.add_parser("extract", help="print one release's notes")
    extract.add_argument("--tag", required=True, help="e.g. v1.2.3")
    extract.add_argument("--output", type=Path, help="write here instead of stdout")
