...
# `mikuswarm` network in docker-compose.yml). This is the egress boundary for the
...
#
# Prereqs: the `mikuswarm` network must exist (docker compose up creates it) and the
...
#
# Usage: sudo MIKUSWARM_AGENT_NETWORK=mikuswarm docker/agent-egress-rules.sh

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec "$HERE/egress-rules.sh" "${MIKUSWARM_AGENT_NETWORK:-mikuswarm}" "mikuswarm-egress" --allow-intra
