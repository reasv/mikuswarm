...
# HARDENING (operator-run, requires root). Restrict the CloakBrowser-Manager
# bridge so the browser can reach the public internet but NOT local/private
# networks (the harness, DB, dev sandbox, other host services, the LAN,
# link-local, loopback). This is the network-layer SSRF boundary for browsing —
# enforced where a JS redirect inside the page cannot bypass it (the tool-layer
# scheme/RFC1918 checks are defense-in-depth on top of this).
#
...
# new bridge and any rules keyed on the old bridge go stale. The flush-by-comment
# below removes our previous rules and re-derives them for the current bridge on
# every run, so re-running is always safe and self-correcting.
#
...
COMMENT="mikuswarm-browser-egress"

# Pinned public resolvers. DNS is permitted to these destinations only — not to
# arbitrary hosts on port 53, which would otherwise allow DNS-tunnel exfil to the
# LAN/harness/DB.
...
# Drop traffic originating from the browser bridge to private ranges.
...
# --- IPv6 ----------------------------------------------------------------
# The bridge is created with IPv6 disabled (ensure-browser-network.sh), so the
# browser should have no IPv6 address. As defense-in-depth, if ip6tables is
...
if command -v ip6tables >/dev/null 2>&1; then
  if ip6tables -L DOCKER-USER -n >/dev/null 2>&1; then
    ...
    echo "         (the browser bridge has IPv6 disabled, so this is expected)" >&2
  fi
else
