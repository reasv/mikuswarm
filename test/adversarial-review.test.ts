  ...
  try {
    ...
    // Reopen → v10→v11 migration runs
    {
      ...
      try {
        ...
        assert.equal(ver, LATEST_SCHEMA_VERSION, "version stamped to 11");
...
// ─────────────────────────────────────────────────────────────────────────────
test("adversarial/schema: LATEST_SCHEMA_VERSION = 11; all four expected indexes present on fresh DB", async () => {
  assert.equal(LATEST_SCHEMA_VERSION, 11, "LATEST_SCHEMA_VERSION is 11");
